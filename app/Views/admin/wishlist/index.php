    ...
    $(document).ready(function() {
        ...
        function performSearch(searchTerm) {
            ...
            if (visibleItems === 0) {
                if (!$noResults.length) {
                    const noResultsHtml = `
                    <div id="noResults" class="col-12 mt-3 animate__animated animate__fadeIn">
                        <div class="card custom-card">
                            <div class="card-body text-center py-4">
                                <i class="ri-search-line fs-3 text-muted mb-3"></i>
                                <h5 class="fw-semibold">No se encontraron productos</h5>
                                <p class="text-muted">Intenta con otros términos de búsqueda</p>
                            </div>
                        </div>
                    </div>`;
                    $('.row').append(noResultsHtml);
                }
            } else {
                $noResults.remove();
            }
        ...
        $('#searchWishlist').on('keyup', function(e) {
            if (e.key === 'Escape') {
                $(this).val('');
                performSearch('');
                $(this).blur();
        ...
        $('#searchButton').on('click', function() {
            const searchTerm = $('#searchWishlist').val();
