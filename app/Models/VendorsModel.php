...
class VendorsModel extends Model
{
   ...
   public function filter($search = null, $limit = null, $start = null, $orderField = null, $orderDir = null)
   {
      ...
      $orderField = in_array($orderField, $this->allowedFields) ? $orderField : 'vendors';
      ...
      foreach ($query as $index => $value) {
         ...
         $query[$index]['column_action'] = '<button class="btn btn-sm  btn-success form-action" item-id="' . $query[$index][$this->primaryKey] . '" purpose="detail"><i class="bi bi-eye"></i></button> <button class="btn btn-sm  btn-warning form-action" purpose="edit" item-id="' . $query[$index][$this->primaryKey] . '"><i class="bi bi-pencil"></i></button>';
      }
      return $query;
   }
